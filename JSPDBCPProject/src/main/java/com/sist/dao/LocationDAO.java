...
public class LocationDAO {
	...
	public void getConnection()
	{
		try
		{
			...
			DataSource ds=(DataSource)c.lookup("jdbc/oracle"); // connection 가져오는 객체 : jdbc/oracle
			conn=ds.getConnection(); //미리 생성된 Connection 객체를 얻어온다
			//DL / DI => 스프링 ==> lookup => 이름으로 객체를 찾아 온다 => 객체마다 id를 부여해서 찾아오는
			// 톰캣 => 보안
		}catch(Exception ex) {}
	}
	// 반환             => disConnection ==> ps.close(), conn.close() 하면 반환됨
	...
	public List<LocationVO> locationListData(int page)
	{
		...
		try
		{
			...
			// rownum을 이용시 => Top-N (중간에 있는 데이터는 가지고 오지 못한다) --> 아예 테이블을 따로 만든다 그래서 from이 길어짐..
