...
public class ReplyBoardDAO {
	...
	// 답변하기 ************* sql 4개가 같이 실행되어야 답변이 올라간다 
	public void replyInsert(int pno,ReplyBoardVO vo)
	{
		 try
		 {
			 ...
			 /*							gi        gs       gt
			  * 	AAAAAA				1         0         0
			  *        => KKKKKK        1         1         1   맨위로 올라오면서 --AAAAAA에 대한 답변 
			  * 	   => BBBBBB        1         1(+1)     1(+1)    밑에꺼가 +1씩 증가--KKKKKK가 이 밑으로 들어간다..오류!!
			  * 		  => CCCCCC		1         2(+1)     2(+1) 
			  * 			 => DDDDD   1         3(+1)     3(+1) 
			  *		 
			  *			이 답변이 안깨지게 만드는 방법
			 ...
			 ps.setInt(1, pno);
			 ps.executeUpdate();
			 // ==================== 일괄처리 (트랜잭션) -- insert,update가 번갈아가며 많이 들어갔다 하면 트랜잭션
			 conn.commit(); // 4개중에 하나라도 오류나면 밑에 롤백하게끔  // 4개가 다 실패하던지 다 성공하든지 //
		 }catch(Exception ex)
		 {
			 try
			 {
		 ...
		 finally
		 {
			 try
			 {
				 conn.setAutoCommit(true); // 트랜잭션!!!(=롤백 or 커밋 둘중하나다)  롤백(전체 취소)전에 맨마지막에 커밋. 
			 }catch(Exception ex) {}
			 disConnection();
		 }
	}
	...
	public ReplyBoardVO boardUpdateData(int no)
	{
		...
		{
			getConnection();
			// 수정하기는 이름 제목 내용만 쓰면 되니깐 나머지는 지웠음
	...
	public String boardDelete(int no,String pwd) // 트랜잭션 구성 잘알아두기
	{
		String result="";
		try
		{
			...
			if(db_pwd.equals(pwd))
			{
				...
				int depth=rs.getInt(2); //삭제냐 아니냐 결정하는 
				rs.close();
