...
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%> <!--제어문이나 화면이동할때 taglib쓴다  -->
...
$(function(){ //window.onload (Java=main())
	...
	$('#delBtn').on("click",function(){
		...
		$.ajax({
			type: 'post',
			url:'delete.jsp',
			data:{"no":no,"pwd":pwd}, //delete.jsp?no=1&pwd=1234 데이터 보냄
			success:function(result) //결과값받기
			{
				let res=result.trim();
				if(res=="yes")
				{
				...
				<table class="table">
					...
					<tr>
						<td colspan="4" class="text-right">
							...
							<a href="update.jsp?no=${vo.no }" class="btn btn-xs btn-info">수정</a> <!--본인꺼 삭제할때는 no로 구분  -->
