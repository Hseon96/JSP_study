...
public class FoodDAO {
	...
	public List<FoodVO> foodListData(String fd,int page)
	{
		...
		try
		{
			...
			while(rs.next())
			{
				FoodVO vo=new FoodVO(); // Bean이 모아주는것 이렇게
	...
	//총페이지 
	public int foodTotalPage(String fd) // finddata 찾는 데이터 줄임말
	{
		int total=0;
		try
		{
	...
	public FoodVO foodDetailData(int no)
	{
		...
		{
			conn=dbconn.getConnection();
			String sql="SELECT fno,name,address,tel,type,price,time,parking,menu,poster,score "
					+ "FROM food_location "
					+ "WHERE fno=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, no);
